...
class Affectdataset(Dataset):
    ...
    def __init__(self, data: Dict, flatten_time_series: bool, aligned: bool = True, task: str = None, max_pad=False, max_pad_num=50, data_type='mosi', z_norm=False, device=None) -> None:
    ...
    def __getitem__(self, ind):
        ...
        # vision = torch.tensor(self.dataset['vision'][ind], device=self.device)
        # audio = torch.tensor(self.dataset['audio'][ind], device=self.device)
        # text = torch.tensor(self.dataset['text'][ind], device=self.device)
        ...
        # label = torch.tensor(_get_class(tmp_label), device=self.device).long() if self.task == "classification" else torch.tensor(
        #     tmp_label, device=self.device).float()
...
def get_dataloader(
        ...
        raw_path='/home/sb/backup/pack/mosi/mosi.hdf5', z_norm=False) -> DataLoader:
    ...
    Args:
        ...
        raw_path (str, optional): Full path to data. Defaults to '/home/sb/backup/pack/mosi/mosi.hdf5'.
...
def get_noise_dataloader(
        filepath: str, batch_size: int = 32, max_seq_len=50, max_pad=False, train_shuffle: bool = True,
        num_workers: int = 2, flatten_time_series: bool = False, task=None, robust_test=False, data_type='mosi', 
        raw_path='/home/sb/backup/pack/mosi/mosi.hdf5', z_norm=False, full_data=None, i=1, device=None) -> DataLoader:
    """Get dataloaders for affect data.
    """
    process = eval("_process_2") if max_pad else eval("_process_1")
    
    if full_data is  not None and robust_test:
        processed_dataset = {'test': {}}
        processed_dataset['test'] = full_data
        processed_dataset['test'] = drop_entry(processed_dataset['test'])

        vids = [id for id in processed_dataset['test']['id']]

        file_type = raw_path.split('.')[-1]  # hdf5
        rawtext, vids = get_rawtext(raw_path, file_type, vids)

        # i = 1 # noise level
        print(f'Noise level = {i/10}...')
        test = dict()
        ...
        # for i in range(10):
        #     test = dict()
        #     test['vision'] = processed_dataset['test']["vision"]
        #     test['audio'] = processed_dataset['test']["audio"]
        test['text'] = _glove_embeddings(add_text_noise(rawtext, noise_level=i / 10), vids)
            # test['labels'] = processed_dataset['test']["labels"]
            # test = drop_entry(test)
        ...
        # for i in range(10):
        #     test = dict()
        test['vision'] = add_timeseries_noise([processed_dataset['test']['vision'].copy()], noise_level=i / 10, rand_drop=False)[0]
            
            # test['audio'] = processed_dataset['test']["audio"].copy()
            # test['text'] = processed_dataset['test']['text'].copy()
            # test['labels'] = processed_dataset['test']["labels"]
            # test = drop_entry(test)
            # print('test entries: {}'.format(test['vision'].shape))
        ...
        # for i in range(10):
        #     test = dict()
        #     test['vision'] = processed_dataset['test']["vision"].copy()
        test['audio'] = add_timeseries_noise([processed_dataset['test']['audio'].copy()], noise_level=i / 10, rand_drop=False)[0]
            # test['text'] = processed_dataset['test']['text'].copy()
            # test['labels'] = processed_dataset['test']["labels"]
            # test = drop_entry(test)
            # print('test entries: {}'.format(test['vision'].shape))
        ...
        # for i in range(10):
        robust_timeseries_tmp = add_timeseries_noise(
                [test['vision'].copy(), test['audio'].copy(), test['text'].copy()],
                noise_level=i / (10 * 3), rand_drop=False)
        ...
        n_samples = 1.0 * len(full_data['labels']) # 2138
        ...
        train_size_ratio = 1283/n_samples
        valid_size_ratio = 214/n_samples
        # test_size_ratio = 1 - train_size_ratio - valid_size_ratio  #686

     
        current_total_samples = len(test['labels'])
        train_size = int(current_total_samples * train_size_ratio)
        valid_size = int(current_total_samples * valid_size_ratio)

      
        indices = np.arange(current_total_samples)
        ...
        train_data = {key: value[train_indices] for key, value in test.items()}
        
        valid_data = {key: value[valid_indices] for key, value in test.items()}
        
        test_data = {key: value[test_indices] for key, value in test.items()}
        
        train_dataloader = DataLoader(Affectdataset(train_data, flatten_time_series, task=task, max_pad=max_pad,  max_pad_num=max_seq_len, data_type=data_type, z_norm=z_norm, device=device), \
                       shuffle=train_shuffle, num_workers=num_workers, batch_size=batch_size, \
                       collate_fn=process)
        
        valid_dataloader = DataLoader(Affectdataset(valid_data, flatten_time_series, task=task, max_pad=max_pad, max_pad_num=max_seq_len, data_type=data_type, z_norm=z_norm, device=device), \
                       shuffle=False, num_workers=num_workers, batch_size=batch_size, \
                       collate_fn=process)
        
        test_dataloader = DataLoader(Affectdataset(test_data, flatten_time_series, task=task, max_pad=max_pad, max_pad_num=max_seq_len, data_type=data_type, z_norm=z_norm, device=device), \
                      shuffle=False, num_workers=num_workers, batch_size=batch_size, \
                      collate_fn=process)
...
def get_fulldata(
    ...
    full_data = {}
    for key in alldata['train'].keys():
        concatenated_data = np.concatenate([alldata[split][key] for split in alldata.keys()],  axis=0)
        np.random.shuffle(concatenated_data)
        full_data[key] =  concatenated_data

    return full_data


def get_fold_dataloader(
        train_data, valid_data, batch_size: int = 32, max_seq_len=50, max_pad=False, train_shuffle: bool = True,
        num_workers: int = 2, flatten_time_series: bool = False, task=None, robust_test=False, data_type='mosi', 
        z_norm=False, device=None) -> DataLoader:
    """
    n-fold cross-validation
  
    """

    processed_dataset = {'train': train_data,'valid': valid_data}
    ...
    train = DataLoader(Affectdataset(processed_dataset['train'], flatten_time_series, task=task, max_pad=max_pad,               max_pad_num=max_seq_len, data_type=data_type, z_norm=z_norm, device=device), \
                       shuffle=train_shuffle, num_workers=num_workers, batch_size=batch_size, \
                       collate_fn=process)
    valid = DataLoader(Affectdataset(processed_dataset['valid'], flatten_time_series, task=task, max_pad=max_pad, max_pad_num=max_seq_len, data_type=data_type, z_norm=z_norm, device=device), \
                       shuffle=False, num_workers=num_workers, batch_size=batch_size, \
                       collate_fn=process)
    return train, valid
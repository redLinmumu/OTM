...
def get_fulldata(data_dir, flatten_audio=False, flatten_image=False, unsqueeze_channel=True, generate_sample=False, normalize_image=True, normalize_audio=True):
...
def get_dataloader_nfold(data_dir, train_index, valid_index, batch_size=40, num_workers=8, train_shuffle=True, flatten_audio=False, flatten_image=False, unsqueeze_channel=True, generate_sample=False, normalize_image=True, normalize_audio=True):
    fulllist = get_fulldata(data_dir=data_dir, flatten_audio=flatten_audio, flatten_image=flatten_image, unsqueeze_channel=unsqueeze_channel, generate_sample=generate_sample, normalize_image=normalize_image, normalize_audio=normalize_audio)
    fulllist = np.array(fulllist, dtype=object)
    valid_data = fulllist[valid_index].tolist()
    train_data = fulllist[train_index].tolist()
    valids = DataLoader(valid_data, shuffle=False,
                        num_workers=num_workers, batch_size=batch_size)

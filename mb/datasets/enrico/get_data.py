...
class EnricoDataset(Dataset):
    ...
    def __init__(self, data_dir, mode="train", noise_level=0, img_noise=False, wireframe_noise=False, img_dim_x=128, img_dim_y=256, random_seed=42, train_split=0.6, val_split=0.2, test_split=0.2, normalize_image=False, seq_len=64):
...
def get_dataloader(data_dir, batch_size=32, num_workers=0, train_shuffle=True, return_class_weights=True):
    ...
    dl_train = DataLoader(ds_train, num_workers=num_workers,
                          sampler=sampler, batch_size=batch_size, shuffle=train_shuffle)
...
class EnricoDatasetMixer(Dataset):
    """Implements torch dataset class for ENRICO dataset."""
    
    def __init__(self, data_dir, input_keys=[], noise_level=0, img_noise=False, wireframe_noise=False, img_dim_x=128, img_dim_y=256, random_seed=42, train_split=0.6, val_split=0.2, test_split=0.2, normalize_image=False, seq_len=64):
        """Instantiate ENRICO dataset.
        """
        super(EnricoDatasetMixer, self).__init__()
        self.noise_level = noise_level
        self.img_noise = img_noise
        self.wireframe_noise = wireframe_noise
        self.img_dim_x = img_dim_x
        ...
        self.keys = np.array(keys)[input_keys].tolist()
...
def get_dataloader_nfold(ds_train: EnricoDatasetMixer, ds_val: EnricoDatasetMixer,  batch_size=32, num_workers=0, train_shuffle=True, return_class_weights=True):
    targets = []
    class_counter = Counter()
    for i in range(len(ds_train)):
        example_topic = ds_train.example_list[ds_train.keys[i]]['topic']

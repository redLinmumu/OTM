...
class PushTask():
    ...
    @classmethod
    def get_fullnoise_test_trajectories(
        cls, full_trajectories, dataset_args
    ):
        """Get test trajectories."""

        i = dataset_args['noise']
        
        print(f'noise level = {i}')
        
        max_trajectory_count = 1310
        
        trajectories = []
        
        for raw_trajectory_index, raw_trajectory in enumerate(full_trajectories):
            if raw_trajectory_index >= max_trajectory_count:
                break
            
            state_base =   raw_trajectory[0]  
            observations_base = raw_trajectory[1]  
            
            observations_noise = {}
            
            controls_base = raw_trajectory[2]
            image_base = observations_base["image"]
            gripper_sensors_base = observations_base['gripper_sensors']
            gripper_pos_base = observations_base['gripper_pos']
            ...
            # multimodal_noise
            tmp = add_timeseries_noise([image_base, gripper_pos_base, gripper_sensors_base, controls_base], noise_level=i/10, rand_drop=False)
            observations_noise["image"] = tmp[0]
            observations_noise["gripper_pos"] = tmp[1]
            observations_noise["gripper_sensors"] = tmp[2]
            controls_noise = tmp[3]    
            
            # trajectories.append(
            #         TrajectoryNumpy(
            #             state_base,
            #             fannypack.utils.SliceWrapper(observations_noise),
            #             controls_noise,
            #         )
            #     )
            trajectories.append(
        ...
        print(f'Noise Added -- type: multimodal')
        return trajectories
...
def get_full_trajectories(dataset_args):
    file_path = 'total_trajectories.pkl'
    if os.path.exists(file_path):
        with open(file_path, 'rb') as f:
            full_trajectories = pickle.load(f)
# n_samples = 1310
    else:
        train = _load_trajectories("gentle_push_1000.hdf5", **dataset_args)
        val = _load_trajectories("gentle_push_10.hdf5", **dataset_args)
        test = _load_trajectories("gentle_push_300.hdf5", **dataset_args)
        full_trajectories = train + val + test
...
import numpy as np
def get_dataloader_nfold(
        subsequence_length: int, train_index: np.ndarray, val_index: np.ndarray, modalities=None, batch_size=32, drop_last=True, **dataset_args
    ):
    if modalities is None:
        full_trajectories = get_full_trajectories()
        
        train_index = train_index.tolist()
        val_index = val_index.tolist()
        
        total_num = len(full_trajectories)
        ...
        for i in range(total_num):
           if i in train_index:
               train_trajectories.append(full_trajectories[i])
           elif i in val_index:
               val_trajectories.append(full_trajectories[i])
        
        device = dataset_args['device']
        
        train_loader = DataLoader(
            SubsequenceDataset(train_trajectories,
                               subsequence_length, modalities, device=device),
            batch_size=batch_size,
            shuffle=True,
            drop_last=drop_last,
        )
        val_loader = DataLoader(
            SubsequenceDataset(
                val_trajectories, subsequence_length, modalities, device=device),
            batch_size=batch_size,
            shuffle=True,
        )
        ...
        print("Error: not added yet!")
        return None, None


def get_dataloader_noise(
        subsequence_length: int,  modalities=None, batch_size=32, drop_last=True, dataset_args=None
    ):
    if modalities is None:
        full_trajectories_base = get_full_trajectories(dataset_args=dataset_args)
        
        # noise = dataset_args['noise']
        
        full_noise_trajectories = PushTask.get_fullnoise_test_trajectories(full_trajectories=full_trajectories_base, dataset_args=dataset_args)
        
        
        total_num = len(full_noise_trajectories)
        train_size = 1000
        valid_size = 10
        ...
        for j in range(total_num):
           if j in train_indices:
               train_trajectories.append(full_noise_trajectories[j])
           elif j in valid_indices:
               val_trajectories.append(full_noise_trajectories[j])
           elif j in test_indices:
               test_trajectories.append(full_noise_trajectories[j])
        ...
        val_loader = DataLoader(
            SubsequenceDataset(val_trajectories, subsequence_length, modalities, device=device),
            batch_size=batch_size,
            shuffle=True,
        )
        
        test_loader = DataLoader(
            SubsequenceDataset(test_trajectories, subsequence_length, modalities, device=device),
            batch_size=batch_size,
            shuffle=False,
        )
...
def split_trajectories(
    ...
    for traj in trajectories:
        ...
        try:
            ...
            assert len(traj.observations) == trajectory_length
            assert len(traj.controls) == trajectory_length
...
class SubsequenceDataset(Dataset):
    ...
    def __init__(
        self, trajectories: List[TrajectoryNumpy], subsequence_length: int, modalities=None, device='cpu'
    ):
    ...
    def __getitem__(self, index: int):
        ...
        if self.device == 'cpu':
            ...
            length=len(self.subsequences[index])
            return tuple(torch.from_numpy(self.subsequences[index][i]).to(self.device) for i in range(length))    

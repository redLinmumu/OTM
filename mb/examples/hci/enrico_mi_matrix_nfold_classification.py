...
for fold, (train_index, valid_index) in enumerate(kfold.split(X=np.zeros(n_samples))):
    ...
    save_as_mixer = pts_dir + 'mi_mixer_fold' + str(fold)+ '.pt'
    save_as_origin = pts_dir + 'mi_origin_fold' + str(fold)+ '.pt'
    ...
    fusion_mix = MultiplicativeInteractions2Modal([16, 16], 32, "matrix").to(device)

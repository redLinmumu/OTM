...
mixer_dir = 'best_pts/enrico/tensor_matrix/' + str(beta) + '/'

single_dir = 'best_pts/enrico/tensor_matrix/single/'
...
for fold, (train_index, valid_index) in enumerate(kfold.split(X=np.zeros(n_samples))):
    ...
    fusion_mixer = MultiplicativeInteractions2Modal([16, 16], 32, "matrix", True).to(device)
    
    mixer_file_path = mixer_dir + 'tensormatrix_mixer_fold' + str(fold)+ '.pt'
    ...
    print("Testing mixer:")
    test(model_mixer, testdata, dataset='enrico', mixer_type='mixer', fold_index=fold, method_name='Tensor')
    ...
    fusion_origin = MultiplicativeInteractions2Modal([16, 16], 32, "matrix", True).to(device)
    
    origin_file_path = mixer_dir + 'tensormatrix_fold' + str(fold)+ '.pt'
    ...
    test(model_origin, testdata, dataset='enrico', mixer_type='origin', fold_index=fold, method_name='Tensor')
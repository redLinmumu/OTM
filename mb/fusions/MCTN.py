...
class Seq2Seq(nn.Module):
    ...
    def forward(self, src, trg, teacher_forcing_ratio=0.5):
        ...
        outputs = Variable(torch.zeros(
            max_len, batch_size, output_size)).to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
        ...
        for t in range(0, max_len):
            ...
            if is_teacher:
                output = Variable(trg.data[t]).to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))

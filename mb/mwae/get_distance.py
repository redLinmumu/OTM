import torch
import ot
from mwae.gw_transport import gromov_wasserstein, fused_gromov_wasserstein
...
def mixer_fuse(zs: list):
    bs = []
    device = zs[0].device
    for i in range(len(zs)):
        zs[i] = zs[i].reshape(zs[i].shape[0], -1)
        bs.append(torch.ones(zs[i].shape[0], device=device) / zs[i].shape[0])
    # z = zs[0].detach()  
    z = zs[0]
    # compute ot plans
    trans = []
    cost1 = torch.cdist(z.detach(), z.detach())
    for i in range(1,len(zs)):
        # cost2 = torch.cdist(zs[i].detach(), zs[i].detach())
        cost2 = torch.cdist(zs[i].detach(), zs[i].detach())
        # cost2 = distance_tensor(pts_src=zs[i], pts_dst=zs[i])
        tran = gromov_wasserstein(C1=cost1, C2=cost2, p=bs[0], q=bs[i], loss_fun='square_loss')
        trans.append(tran)
    return trans

def cal_spectral_loss(cluster_num=7, out=None):

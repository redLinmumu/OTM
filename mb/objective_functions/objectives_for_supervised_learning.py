...
import torch
from objective_functions.recon import recon_weighted_sum, recon_weighted_sum_single, elbo_loss
...
def _criterioning(pred, truth, criterion, device=device_default):
...
def MFM_objective(ce_weight, modal_loss_funcs, recon_weights, input_to_float=True, criterion=torch.nn.CrossEntropyLoss(), device=device_default):
    ...
    recon_loss_func_single = recon_weighted_sum_single(modal_loss_funcs, recon_weights)
    def _actualfunc(pred, truth, args):
        reps = args['reps']
        fused = args['fused']
        inps = args['inputs']
        
        if 'intermediates' in args.keys():
            ...
            ce_loss = _criterioning(pred, truth, criterion, device=device)
            if input_to_float:
            ...
            int =  args['intermediate']
            decoder = args['decoder']
            recon = decoder(torch.cat([int(reps), fused], dim=1))
            ce_loss = _criterioning(pred, truth, criterion, device=device)
            inp = inps[args["modality_indx"]]
            if input_to_float:
                input = inp.float().to(device)
            else:
                input = inp.to(device)
            recon_loss = recon_loss_func_single(recon, input)
            return ce_loss*ce_weight+recon_loss
    return _actualfunc
...
def MVAE_objective(ce_weight, modal_loss_funcs, recon_weights, input_to_float=True, annealing=1.0, criterion=torch.nn.CrossEntropyLoss(), device=device_default):
    ...
    def _actualfunc(pred, truth, args):
        ...
        total_loss += ce_weight * _criterioning(pred, truth, criterion, device=device)
        return total_loss
    return _actualfunc



def CCA_objective(out_dim, cca_weight=0.001, criterion=torch.nn.CrossEntropyLoss(), device=device_default):
    """
...
def RefNet_objective(ref_weight, criterion=torch.nn.CrossEntropyLoss(), input_to_float=True, device=device_default):
    """
    ...
    def _actualfunc(pred, truth, args):
        ...
        if input_to_float:
            inputs = [torch.flatten(t, start_dim=1).float().to(device)
                      for t in inps]
        else:
        ...
        for i in range(len(inps)):
            ...
            ss_loss += ss_criterion(out,
                                    inputs[i], torch.ones(out.size(0)).to(device))
        return ce_loss + ss_loss*ref_weight
    return _actualfunc


def RMFE_object(reg_weight=1e-10, criterion=torch.nn.BCEWithLogitsLoss(), is_packed=False, device=device_default):
    """

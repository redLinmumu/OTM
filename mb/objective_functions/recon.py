...
def recon_weighted_sum_single(modal_loss_func, weight):
    """Create wrapper function that computes the weighted model reconstruction loss."""
    def _actualfunc(recon, orig):
        trg = orig.view(recon.shape[0], recon.shape[1]) if len(
                recon.shape) != len(orig.shape) else orig
        totalloss = modal_loss_func(recon, trg) * weight
        return torch.mean(totalloss)
    return _actualfunc